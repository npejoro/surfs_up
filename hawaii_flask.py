...
def prcp_or_temps(choice):
    ...
    most_current = session.query(Measurements.date).order_by(
        Measurements.date.desc()).first()
    last_date = most_current[0]
    year_before = last_date.replace(year=(last_date.year - 1))
    year_before = year_before.strftime("%Y-%m-%d")
    #query
    twelve_months = session.query(Stations.name, Stations.station, Measurements.date, choice).filter(
        Stations.station == Measurements.station, Measurements.date > year_before).order_by(Measurements.date.desc()).all()
    ...
    for x in range(1, len(twelve_months)):
        ...
        record_dict = {"station_id": twelve_months[x-1][1], "station name": twelve_months[x-1]
                       [0], "station %s measure" % (choice): twelve_months[x-1][3]}
        ...
        if x == (len(twelve_months)-1):
            record_dict = {
                "station_id": twelve_months[x][1], "station name": twelve_months[x][0], "station prcp measure": twelve_months[x][3]}
            record_list.append(record_dict)
...
def stations():
    ...
    for station in stations:
        station_dict = {"station_id": station[0], "name": station[1],
                        "latitude": station[2], "longitude": station[3], "elevation": station[4]}
...
def temp_stats(start_date):
    ...
    temps = session.query(func.min(Measurements.tobs), func.max(Measurements.tobs), func.avg(
        Measurements.tobs)).filter(Measurements.date >= start_date).first()
    #create dictionary from result
    temps_dictionary1 = {
        "minimum temperuture": temps[0], "maximum temperature": temps[1], "average temperature": temps[2]}
...
def temp_range(start_date, end_date):
    #query
    temps = session.query(func.min(Measurements.tobs), func.max(Measurements.tobs), func.avg(
        Measurements.tobs)).filter(Measurements.date >= start_date, Measurements.date <= end_date).first()
    #create dictionary from result
